...
static request waitForClientOrChef()
{
    ...
    if ( semDown (semgid, sh->waiterRequest))  {                                                  /* enter critical region */
        perror ("error on the up operation for semaphore access (WT)");
        exit (EXIT_FAILURE);
    }
...
static void informChef (int n)
{
    ...
    if (semUp(semgid, sh->requestReceived[n]))  {                                                  /* enter critical region */
        perror ("error on the up operation for semaphore access (WT)");
        exit (EXIT_FAILURE);
    }
    if (semUp (semgid, sh->waitOrder))  {                                                  /* enter critical region */
        perror ("error on the up operation for semaphore access (WT)");
        exit (EXIT_FAILURE);
    }
...
static void takeFoodToTable (int n)
{
    ...
    saveState(nFic, &sh->fSt);
    if (semUp (semgid, sh->foodArrived[n]))  {                                                  /* enter critical region */
        perror ("error on the up operation for semaphore access (WT)");
        exit (EXIT_FAILURE);
    }
    if (semUp (semgid, sh->waiterRequestPossible))  {                                                  /* enter critical region */
