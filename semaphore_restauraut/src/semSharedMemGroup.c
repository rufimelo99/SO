...
static void checkInAtReception(int id)
{
    ...
    if (semDown (semgid, sh->receptionistRequestPossible)) {                                                  /* enter critical region */
        perror ("error on the down operation for semaphore access (CT)");
        exit (EXIT_FAILURE);
    }
    //waitForTable  
    if (semDown (semgid, sh->waitForTable[id])) {                                                  /* enter critical region */
        perror ("error on the down operation for semaphore access (CT)");
        exit (EXIT_FAILURE);
    }
...
static void orderFood (int id)
{
    ...
    // TODO insert your code here
    if (semUp (semgid, sh->waiterRequest)) {                                                  /* enter critical region */
        perror ("error on the down operation for semaphore access (CT)");
        exit (EXIT_FAILURE);
    }
    if (semDown (semgid, sh->waiterRequestPossible)) {                                                  /* enter critical region */
        perror ("error on the down operation for semaphore access (CT)");
        exit (EXIT_FAILURE);
    }

    //FOODREQ   3
    sh->fSt.receptionistRequest.reqType = FOODREQ;
    


    int nt=sh->fSt.assignedTable[id]; 
    if (semDown(semgid, sh->requestReceived[nt])) {                                                  /* enter critical region */
        perror ("error on the down operation for semaphore access (CT)");
        exit (EXIT_FAILURE);
    }
...
static void waitFood (int id)
{
    ...
    // TODO insert your code here
    int nt=sh->fSt.assignedTable[id];
    if (semDown (semgid, sh->foodArrived[nt])) {                                                  /* enter critical region */
        perror ("error on the down operation for semaphore access (CT)");
        exit (EXIT_FAILURE);
    }
    ...
    saveState(nFic, &sh->fSt);
    // foodArrived[NUMTABLES] (do grupo fica a 0?)
    ...
    // TODO insert your code here       ??
    if (semUp (semgid, sh->waiterRequestPossible)) {                                                  /* enter critical region */
        perror ("error on the down operation for semaphore access (CT)");
        exit (EXIT_FAILURE);
    }
...
static void checkOutAtReception (int id)
{
    ...
    // TODO insert your code here
    // tableDone[NUMTABLES] ---> fica a 1 
    int nt=sh->fSt.assignedTable[id];
    
    
    if (semDown (semgid, sh->tableDone[nt])) {                                                  /* enter critical region */
        perror ("error on the down operation for semaphore access (CT)");
        exit (EXIT_FAILURE);
    }
    ...
    // TODO insert your code here
    if (semUp (semgid, sh->receptionistRequestPossible)) {                                                  /* enter critical region */
        perror ("error on the down operation for semaphore access (CT)");
        exit (EXIT_FAILURE);
    }
